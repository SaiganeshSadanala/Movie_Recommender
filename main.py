...
if st.button('Search'):
    with st.spinner("Fetching recommendations..."):
        time.sleep(2)
    names, posters, info , cast= recommend(selected_movie)
    st.markdown(f"<h2>Selected Movie: {selected_movie}</h2>", unsafe_allow_html=True)
    ...
    with col2:
        movie_expander = st.expander(label=f"Details of {selected_movie}", expanded=True, )
        with movie_expander:
            st.write(f"{selected_movie_data.overview}")
    st.markdown(f"**Recommendations for movie:** {selected_movie}")
